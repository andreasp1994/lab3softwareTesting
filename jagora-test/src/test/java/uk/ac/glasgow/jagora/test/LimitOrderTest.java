...
public class LimitOrderTest {
	...
	public void testBadSatisfyTradeQuantityNegative() throws Exception {
		badTrade = new StubTrade(-1, 5.0, stock);
		tickEvent = new StubTickEvent<Trade>(badTrade, 0L);
		sellOrder.satisfyTrade(tickEvent);
	}
	
	@Test(expected=TradeException.class)
	public void testBadSatisfyTradeQuantityZero() throws Exception {
		badTrade = new StubTrade(0, 5.0, stock);
		tickEvent = new StubTickEvent<Trade>(badTrade, 0L);
		sellOrder.satisfyTrade(tickEvent);
	}
	...
	public void testRollBackTrade() throws Exception {
		...
		Integer initialInventory = trader.getInventoryHolding(stock);
		...
		assertTrue("Trader's cash was not properly restored", initialCash  == trader.getCash());
		assertTrue("Trader's inventory was not restored",
				initialInventory == trader.getInventoryHolding(stock));
	}
