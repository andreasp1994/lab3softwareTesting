...
public class ContinuousOrderDriverMarketTest {
	...
	protected World stubWorld = StubWorld.world;
	...
	public void testClearingPossibleTradesExist() {
		...
		sellOrder = new StubSellOrder(StubTrader.seller, lemons, 5, 4.0);
		market.placeSellOrder(sellOrder);
		sellOrder = new StubSellOrder(StubTrader.seller, lemons, 16, 5.0);
		market.placeSellOrder(sellOrder);
		...
		List<TickEvent<Trade>> expectedResult = Arrays.asList(
				new StubTickEvent<Trade>(new StubTrade(5, 4.0, lemons), 0L),
				new StubTickEvent<Trade>(new StubTrade(15, 5.0, lemons), 0L),
				new StubTickEvent<Trade>(new StubTrade(1, 5.0, lemons), 0L));
	...
	public void testClearingNoPossibleTrades() {
		...
		buyOrder = new StubBuyOrder(StubTrader.buyer, lemons, 3, 5.0);
		market.placeBuyOrder(buyOrder);
		sellOrder = new StubSellOrder(StubTrader.seller, lemons, 3, 10.0);
		market.placeSellOrder(sellOrder);
		
		assertTrue("No trades should be made", market.doClearing().isEmpty());
	}
	...
	public void testFullTradeClearing() {
		...
		buyOrder = new StubBuyOrder(StubTrader.buyer, lemons, 2, 5.0);
		market.placeBuyOrder(buyOrder);
		sellOrder = new StubSellOrder(StubTrader.seller, lemons, 2, 5.0);
		market.placeSellOrder(sellOrder);
		
		List<TickEvent<Trade>> returnResult = market.doClearing();
		List<TickEvent<Trade>> expectedResult = Arrays.asList(
				new StubTickEvent<Trade>(new StubTrade(2, 5.0, lemons), 0L));
